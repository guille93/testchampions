...
class ChampionsPorraApp {
    ...
    // Predictions Viewer Integration
    transformToMatchesData() {
        // Transform matches and predictions into matchesData format
        ...
        matches.forEach(match => {
            const date = match.datetime.split('T')[0]; // Extract YYYY-MM-DD
        ...
        // Convert to array similar to predictions-viewer format
        const matchesData = [];
        Object.keys(matchesByDate).sort().forEach(date => {
            matchesByDate[date].forEach(match => {
                const predictionsArray = participants.map(participant => {
                    const pred = predictions[participant] && predictions[participant][match.id];
                    if (pred && pred.predHome !== null && pred.predAway !== null) {
                        // Determine sign: 1, X, 2
                        let sign = 'X';
                        if (pred.predHome > pred.predAway) sign = '1';
                        else if (pred.predHome < pred.predAway) sign = '2';
                        return `${sign}|${pred.predHome}-${pred.predAway}`;
                    } else {
    ...
    renderPronosticosTab() {
        const matchesData = this.transformToMatchesData();
        const participants = this.data.participants;
        ...
        if (matchesData.length === 0) {
            container.innerHTML = '<div class="no-matches">No hay partidos disponibles</div>';
            return;
        }
        ...
        matchesData.forEach(match => {
            ...
            matchCard.appendChild(predictionsGrid);
    ...
    generateDateFilterOptions(matchesData) {
    ...
    filterPredictions() {
        ...
        const matchCards = document.querySelectorAll('#predictionsContainer .match-card');
        ...
        const container = document.getElementById('predictionsContainer');
        if (!hasVisibleMatches && container) {
